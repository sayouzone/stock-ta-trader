"""
ta_trader/llm/growth_prompt_builder.py
GrowthAnalysisResult → LLM 프롬프트 변환기
...
class GrowthPromptBuilder(BasePromptBuilder[GrowthAnalysisResult]):
    ...
    @staticmethod
    def _section_indicators(d: GrowthAnalysisResult) -> str:
        lines = ["## 기술적 지표 상세"]
        return d.screening.detail

    @staticmethod
    def _section_risk(d: GrowthAnalysisResult) -> str:
        if not d.position:
            return ""
