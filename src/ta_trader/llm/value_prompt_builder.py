"""
ta_trader/llm/value_prompt_builder.py
ValueAnalysisResult → LLM 프롬프트 변환기
...
class ValuePromptBuilder(BasePromptBuilder[ValueAnalysisResult]):
    """TradingDecision + DataFrame → LLM 입력 프롬프트 생성"""

    # ── 섹션 빌더 ────────────────────────────────────────

    @staticmethod
    def _section_header(d: ValueAnalysisResult) -> str:
        return (
            f"## 분석 대상\n"
            f"- 티커: {d.ticker}\n"
            f"- 분석 기준일: {d.date}\n"
    ...
    @staticmethod
    def _section_indicators(d: ValueAnalysisResult) -> str:
        lines = ["## 기술적 지표 상세"]
        return d.screening.detail

    @staticmethod
    def _section_risk(d: ValueAnalysisResult) -> str:
        if not d.position:
            return ""
