...
from ta_trader.models import MarketRegime, StrategyType, WeightSet
...
def detect_regime(row: pd.Series, prev_rows: pd.DataFrame | None = None) -> RegimeContext:
    """
...
def _classify_matrix(
    ...
    # 2. 횡보 + 변동성 수축 → 돌파 대기
    if adx < ADX_WEAK_TREND and is_squeeze:
    ...
    if adx < ADX_STRONG_TREND and is_squeeze:
        detail = (
    ...
    # 4. 횡보 + 변동성 확대 → 평균회귀
    if adx < ADX_WEAK_TREND and is_expanding:
        detail = (
