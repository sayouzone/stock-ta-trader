...
def format_position_report(results: list[PositionAnalysisResult]) -> str:
    ...
    header = (
        ...
        f"{'섹터':^10} {'등급':^4} {'손절':>10} {'익절':>10} "
        f"{'RR':>5} {'수량':>10} {'종합':^14} {'점수':>6} {'종목':<0}"
    )
    lines.append(header)
    lines.append("─" * W)
    """
    ...
    for r in sorted_results:
        ...
        line = (
            ...
            f"{r.current_price:>10,.0f} {r.market_env.environment.value:^10} {r.sector.strength.value:^10} "
            f"{r.screening.grade.value:^4} {r.risk.stop_loss:>10,.0f} "
            f"{r.risk.take_profit:>10,.0f} {r.risk.risk_reward_ratio:>5.1f} "
            f"{r.risk.position_size:>10,} {sig_emoji}{r.overall_signal.value:^8} "
            f"{r.overall_score:>6.1f}  {r.name}"
        )
        lines.append(line)
        """
        values = [
            ...
            f"{r.risk.stop_loss:,.0f}",                   # 손절
            f"{r.risk.take_profit:,.0f}",                 # 익절
            f"{r.risk.risk_reward_ratio:.1f}",            # RR
            f"{r.risk.position_size:,}",                  # 수량
