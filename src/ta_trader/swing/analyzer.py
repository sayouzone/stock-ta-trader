"""
ta_trader/swing/analyzer.py
스윙 트레이딩 6단계 프로세스 분석기

6단계 프로세스:
  1. 시장 환경 판단  (ADX, SMA200, ATR%)
  2. 종목 스크리닝   (거래량, RS, +DI/-DI, 정배열)
  3. 진입 타이밍     (MACD, RSI, BB, 피보나치, EMA 크로스)
  4. 포지션 사이징   (ATR 기반 손절/익절, R배수, 자본 배분)
  5. 익절/청산 전략  (트레일링 스톱, RSI/MACD/BB 청산 신호)
  6. 매매 복기       (결과 요약)
"""
...
class SwingTradingAnalyzer:
    ...
    def analyze(self) -> SwingAnalysisResult:
        ...
        # 2단계: 종목 스크리닝
        screening = self._step2_screening(latest, df)

        # 3단계: 진입 타이밍
        entry = self._step3_entry_signal(latest, prev, df)

        # 4단계: 포지션 사이징
        position = self._step4_position_sizing(latest, entry, df)

        # 5단계: 익절/청산 전략
        exit_strategy = self._step5_exit_strategy(latest, prev, position)
    ...
    def _build_summary(
        ...
        signal_desc = {
            SwingSignal.STRONG_ENTRY: "강력한 스윙 매수 신호입니다. 적극 진입을 고려하세요.",
