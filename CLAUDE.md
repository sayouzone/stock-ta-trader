...
│   ├── agents/
│   │   ├── __init__.py
│   │   ├── data.py              ← Data & Analysis Agent — 시장의 모든 정보를 수집하고 정량적/정성적 지표로 가공
│   │   ├── execution.py         ← Execution Agent — 시장에 직접 주문을 넣고 관리
│   │   ├── orchestrator.py      ← Agent Orchestrator — 4-에이전트 파이프라인 총괄 지휘
│   │   ├── risk.py              ← Risk Management Agent — 감정에 휘둘리지 않고 자본을 보호
│   │   └── strategy.py          ← Strategy & Decision Agent — 분석된 데이터를 바탕으로 매매 전략을 수립
│   ├── analyzers/
│   │   ├── __init__.py
│   │   ├── analyzer.py          ← 하위 호환성 유지용 모듈 (Deprecated)
│   │   ├── anthropic.py         ← Anthropic Claude API 기반 기술적 분석 해석기
│   │   ├── google.py            ← Google Gemini API Provider (google-genai SDK)
│   │   ├── growth.py            ← GrowthMomentumAnalyzer — 100% 상승 후보 발굴 6단계 분석 엔진
│   │   ├── position.py          ← PositionTradingAnalyzer — 포지션 트레이딩 7단계 프로세스 분석기
│   │   ├── short.py             ← ShortTermAnalyzer - 단기 트레이딩 분석기
│   │   ├── swing.py             ← SwingTradingAnalyzer - 스윙 트레이딩 6단계 프로세스 분석기
│   │   └── value.py             ← ValueInvestingAnalyzer — 가치 투자 5단계 분석 엔진
│   ├── backtest/
│   │   ├── __init__.py
│   │   ├── engine.py            ← 백테스팅 엔진
│   │   ├── metrics.py           ← 백테스팅 성과 지표 계산
│   │   └── report.py            ← 백테스팅 결과 터미널 보고서 포매터
│   ├── base/
│   │   ├── __init__.py
│   │   ├── agent.py             ← 에이전트 기반 추상 클래스
│   │   ├── analyzer.py          ← 분석기 기반 추상 클래스
│   │   ├── llm.py               ← LLM 분석기 공통 인터페이스 (추상 기반 클래스)
│   │   └── prompt_builder.py    ← LLM 프롬프트 변환기 공통 인터페이스 (추상 기반 클래스)
│   ├── data/
│   │   ├── fetcher.py           ← yfinance 데이터 수집
│   │   └── krx_stock_fetcher.py ← KRX 종목 데이터 로더 및 Yahoo Finance 티커 변환기
│   ├── formatters/
│   │   ├── __init__.py
│   │   ├── agent.py            ← 에이전트 파이프라인 결과 터미널 포매터
│   │   ├── growth.py           ← GrowthAnalysisResult 터미널 출력 포매터
│   │   ├── position.py         ← 포지션 트레이딩 7단계 분석 결과 포매터
│   │   ├── short.py            ← 단기 트레이딩 분석 결과 터미널 출력 포매터
│   │   ├── swing.py            ← 스윙 트레이딩 6단계 분석 결과 포매터
│   │   └── value.py            ← ValueAnalysisResult 터미널 출력 포매터
│   ├── indicators/
│   │   ├── adx.py               ← ADX 신호 분석
│   │   ├── atr.py               ← ATR (Average True Range) 지표 분석 모듈
│   │   ├── bollinger.py         ← Bollinger Bands 신호 분석 모듈
│   │   ├── calculator.py        ← 모든 기술적 지표를 DataFrame에 추가하는 계산기
│   │   ├── fibonacci.py         ← 피보나치 되돌림/확장 분석 모듈
│   │   ├── macd.py              ← MACD 신호 분석
│   │   ├── moving_avg.py        ← 이동평균선 분석 모듈
│   │   ├── rsi.py               ← RSI 신호 분석
│   │   ├── swing_calculator.py  ← 스윙 트레이딩용 확장 지표 계산기
│   │   └── volume.py            ← 거래량 분석 모듈
│   ├── llm/                           ← LLM 기반 자연어 분석 모듈 ★
│   │   ├── __init__.py
│   │   ├── factory.py                 ← LLM Provider 팩토리
│   │   ├── growth_prompt_builder.py   ← GrowthAnalysisResult → LLM 프롬프트 변환기
│   │   ├── position_prompt_builder.py ← PositionAnalysisResult → LLM 프롬프트 변환기
│   │   ├── short_prompt_builder.py    ← TradingDecision → LLM 프롬프트 변환
│   │   ├── swing_prompt_builder.py    ← SwingAnalysisResult → LLM 프롬프트 변환기
│   │   └── value_prompt_builder.py    ← ValueAnalysisResult → LLM 프롬프트 변환기
│   ├── models/
│   │   ├── __init__.py
│   │   ├── agent.py             ← 에이전트 간 통신 데이터 모델
│   │   ├── backtest.py          ← 백테스팅 도메인 모델
│   │   ├── base.py              ← 투자 단계 기본 분석 결과 모델
│   │   ├── growth.py            ← 100% 상승 후보 발굴 6단계 분석 결과 모델
│   │   ├── llm.py               ← LLM 분석 결과 데이터 모델
│   │   ├── position.py          ← 포지션 트레이딩 7단계 분석 데이터 모델
│   │   ├── recommend.py         ← 종목 추천 도메인 모델
│   │   ├── short.py             ← 도메인 데이터 모델 (dataclass 기반)
│   │   ├── swing.py             ← 스윙 트레이딩 6단계 분석 데이터 모델
│   │   └── value.py             ← 가치 투자 5단계 분석 결과 모델
│   ├── signals/
...
- ShortTermAnalyzer       ← 단기 트레이딩 진입점 (ticker 입력 → TradingDecision 반환)
- SwingTradingAnalyzer    ← 스윙 트레이딩 6단계 분석 진입점 (ticker 입력 → SwingAnalysisResult 반환)
- PositionTradingAnalyzer ← 포지션 트레이딩 7단계 분석 진입점 (ticker 입력 → PositionAnalysisResult 반환)
- GrowthMomentumAnalyzer  ← 성장 트레이딩 (100% 상승) 후보 발굴 6단계 분석 진입점 (ticker 입력 → GrowthAnalysisResult 반환)
- ValueInvestingAnalyzer  ← 가치 투자 5단계 분석 진입점 (ticker 입력 → ValueAnalysisResult 반환)
...
- 새 분석기는 반드시 BaseAnalyzer 상속
- 새 에이전트는 반드시 BaseAgent 상속
- 새 LLM 프롬프트 변환기는 반드시 BasePromptBuilder 상속
...
# 마켓별 종목 분석 (KOSPI, KOSDAQ, KRX, US)
python main.py analyze KOSPI
...
# 마켓별 포트폴리오 추천 종목
python main.py recommend -style swing KOSPI
...
## IMPORTANT
- yfinance 데이터 갭 감지 시 upsert 파이프라인 통해서만 처리
- PostgreSQL upsert는 ON CONFLICT DO UPDATE 패턴 사용
